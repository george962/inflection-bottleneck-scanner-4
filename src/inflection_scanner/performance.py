...
def build_track_record(warehouse,horizons_days=None,model_version: str = MODEL_VERSION,benchmark: str="SPY",ledger_path: str | Path | None="published/decision_ledger.jsonl",now: datetime | None=None) -> dict[str,Any]:
    ...
    return {"model_version":model_version,"generated_at":now.isoformat(timespec="seconds"),"benchmark":benchmark,"observations":observations,"summaries":summaries,"note":"V5.4.1 track record is cohort-versioned and can rebuild from the durable committed decision ledger even if the Actions warehouse cache is lost."}
